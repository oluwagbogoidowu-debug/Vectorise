...
export const pushNotificationService = {
  shouldShowPermissionRequest: (user: Participant) => {
    if (user.fcmToken || user.notificationsDisabled) return false;
  ...
  subscribeUser: async (userId: string) => {
    ...
    try {
      ...
      if (!messagingInstance) {
        throw new Error('Firebase Cloud Messaging is not supported or initialized in this browser environment');
      }

      // 5. Get the FCM Token
      console.log('[PushService] Retrieving FCM registration token...');
      const fcmToken = await getToken(messagingInstance, {
        vapidKey: publicKey.trim(),
