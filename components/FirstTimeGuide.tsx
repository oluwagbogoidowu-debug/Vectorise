...
const FirstTimeGuide: React.FC<FirstTimeGuideProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-primary text-white p-4 md:p-8 animate-fade-in overflow-hidden">
      {/* Cancel Button */}
      <button 
        onClick={handleDismiss}
        className="absolute top-6 right-6 z-[110] p-2.5 bg-white/10 hover:bg-white/20 rounded-full transition-all border border-white/10 group"
      >
      ...
      <div className="max-w-sm w-full h-full flex flex-col justify-between relative z-10">
        <header className="flex flex-col items-center pt-4">
          <LocalLogo type="white" className="h-10 w-auto mb-4 opacity-80" />
          ...
          <p className="text-[9px] font-black uppercase tracking-[0.3em] mt-3 opacity-40">Step {currentStep + 1} of {GUIDE_STEPS.length}</p>
        </header>

        <main className="flex-1 flex flex-col items-center justify-center text-center px-4 animate-slide-up" key={currentStep}>
          <div className={`w-20 h-20 ${step.color} rounded-2xl flex items-center justify-center text-4xl mb-6 shadow-2xl border-4 border-white/10`}>
            {step.icon}
          </div>
          <h2 className="text-3xl font-black mb-3 tracking-tight leading-tight">{step.title}</h2>
          <p className="text-base text-white/70 font-medium leading-relaxed italic">"{step.description}"</p>
        </main>
        ...
        <footer className="pb-8">
          <div className="flex gap-3 mb-6">
            {currentStep > 0 ? (
              <button 
                onClick={handleBack}
                className="flex-1 py-4 bg-white/10 backdrop-blur-md text-white border border-white/10 font-black rounded-full text-xs uppercase tracking-widest active:scale-95 transition-all"
              >
                Back
              </button>
            ) : (
            ...
            <button 
              onClick={handleNext}
              className="flex-[2] py-4 bg-white text-primary font-black rounded-full text-xs uppercase tracking-widest shadow-2xl active:scale-95 transition-all hover:bg-gray-50"
            >
      ...
      {/* Decorative Blurs */}
      <div className={`absolute top-0 right-0 w-80 h-80 ${step.color} opacity-20 rounded-full blur-[100px] transition-colors duration-1000 -mr-40 -mt-40`}></div>
