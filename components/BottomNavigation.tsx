...
const BottomNavigation: React.FC = () => {
  ...
  const navItems = [
    {
      ...
      icon: (active: boolean) => (
        <div className="relative">
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${active ? 'text-primary' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    },
    {
      ...
      icon: (active: boolean) => (
        <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${active ? 'text-primary' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={active ? 2.5 : 2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012v2M7 7h10" />
        </svg>
      )
    },
    {
  ...
  return (
    <nav className="fixed bottom-0 left-0 z-50 w-full bg-white border-t border-gray-100 shadow-[0_-1px_6px_rgba(0,0,0,0.02)] safe-area-pb">
      <div className="flex justify-around items-center h-14 max-w-lg mx-auto pb-1 px-4">
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span className={`text-[8.5px] font-black uppercase tracking-[0.05em] transition-colors duration-300 ${isActive ? 'text-primary opacity-100' : 'text-gray-400 opacity-60'}`}>
                  {item.label}
                </span>
              </>
            )}
