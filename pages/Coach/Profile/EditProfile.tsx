...
const EditProfile: React.FC = () => {
  ...
  const [bio, setBio] = useState((user as Coach)?.bio || '');
  const [niche, setNiche] = useState((user as Coach)?.niche || '');
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await updateProfile(sanitizeData({ name, bio, niche }));
      navigate(-1);
    } catch (err) {
    ...
    <div className="bg-[#FDFDFD] h-screen w-full font-sans overflow-hidden flex flex-col animate-fade-in">
      ...
      <main className="flex-1 overflow-y-auto p-8 space-y-10">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest px-1">Bio</label>
            <textarea 
              value={bio}
          ...
          <div className="space-y-2">
            <label className="text-[8px] font-black text-gray-400 uppercase tracking-widest px-1">Niche</label>
            <input 
              value={niche}
