      ...
      <div className="relative w-full h-80 md:h-[480px] bg-gray-100 flex-shrink-0">
        ...
        <div className="absolute bottom-0 inset-x-0 p-6 md:p-12 max-w-4xl mx-auto text-white flex flex-col justify-end h-full w-full">
          ...
          <div className="flex items-center gap-3 pt-2 text-[10px] font-black text-white/80 uppercase tracking-widest border-t border-white/20">
            ...
            <span className="px-2.5 py-1 bg-white/15 rounded-lg text-white">
                                Author</span>
...
const IgnitePlayer: React.FC<{
  ...
  const activeSlides = slides.length >
                                 0 ? slides : ["Type some inspiration to preview!"];
  ...
  const handlePrev = (e: React.MouseEvent) =>
    ...
    e.stopPropagation();
    setActiveSlide(curr =>
                                 (curr - 1 + activeSlides.length) % activeSlides.length);
  };
    ...
    >
      ...
      <div className="flex justify-between items-center z-[410] px-4 font-black tracking-widest text-[10px] text-white/60 uppercase">
        ...
        <span>
                                Ignite Post</span>
        <span>
                                Slide {activeSlide + 1} of {activeSlides.length}</span>
      </div>
...
const EditBlogModal: React.FC<{
  ...
  const renderFormattedPreview = (content: string) =>
                                 {
    if (!content.trim()) {
      return <p className="text-gray-300 italic text-sm">
                                Preview content will appear here in real-time...</p>;
    }
        ...
        <div className="h-1/2 w-full border-b border-gray-100 bg-white p-6 md:p-8 overflow-y-auto flex flex-col space-y-6">
          ...
          <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center justify-between shadow-sm animate-fade-in shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-9 h-9 rounded-xl bg-emerald-500/15 flex items-center justify-center text-base">
                                🌱</div>
              <div>
                                
                <span className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block">
                                88% Complete</span>
                <span className="text-[8px] text-emerald-600 font-bold uppercase tracking-wider block mt-0.5">
                                0 more steps to go</span>
              </div>
                                
            </div>
                                
            <span className="px-2.5 py-1 bg-[#0E7850] text-white rounded-full text-[8px] font-black uppercase tracking-widest">
                                Ready to go</span>
          </div>
          ...
          <div className="flex-1 flex flex-col min-h-[350px]">
            ...
            <div className="flex justify-between items-center mb-1.5">
              ...
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">
                                Blog Post Body</label>
              <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">
                                Single formatted input</span>
            </div>
...
const EditIgniteModal: React.FC<{
  ...
  const handleSubmit = (e: React.FormEvent) =>
    ...
    const cleanSnippet = body.substring(0, 30) + (body.length >
                                 30 ? '...' : '');
    onSave({
      title: cleanSnippet,
      igniteBody: body,
    ...
    <div className="fixed inset-0 z-[300] bg-gray-50 flex flex-col overflow-y-auto animate-fade-in font-sans">
      ...
      <div className="bg-white border-b border-gray-150 px-8 py-5 sticky top-0 flex justify-between items-center z-10 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-black text-primary uppercase tracking-[0.25em]">
                                Ignite Studio</span>
          <span className="text-xs text-gray-400 font-extrabold px-3 py-1 bg-gray-50 rounded-lg">
                                Full Bleed Studio</span>
        </div>
      ...
      <div className="flex-1 max-w-4xl w-full mx-auto p-6 md:p-12 space-y-8 animate-slide-up">
        ...
        <div className="bg-white rounded-[3.5rem] p-8 md:p-12 border border-gray-100 shadow-sm space-y-6">
          ...
          <div>
            ...
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">
                                Inspire someone today (Double break splits slides)</label>
            <textarea 
              value={body} 
              onChange={e =>
                                 setBody(e.target.value)} 
          ...
          <div>
            ...
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">
                                Set Background Color theme</label>
          ...
          <div>
            ...
            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">
                                Live Date (Optional)</label>
            <input 
              type="date" 
              value={igniteDate} 
              onChange={e =>
                                 setIgniteDate(e.target.value)} 
            ...
            <p className="text-[10px] text-gray-400 font-extrabold mt-1.5 uppercase tracking-widest leading-none">
                                Schedule this Ignite to go live on a specific day. If left blank, it comes live immediately.</p>
          </div>
...
const CoachSprints: React.FC = () =>
  ...
  const counts = useMemo(() =>
    ...
    return {
      ...
      published: tFiltered.filter(s =>
                                 s.published).length,
      pending: tFiltered.filter(s =>
                                 s.approvalStatus === 'pending_approval').length,
      rejected: tFiltered.filter(s =>
                                 s.approvalStatus === 'rejected').length,
      draft: tFiltered.filter(s =>
                                 s.approvalStatus === 'draft').length,
    };
  }, [tFiltered]);
  ...
  const filteredSprints = useMemo(() =>
    ...
    if (filter === 'published') filtered = tFiltered.filter(s =>
                                 s.published);
    else if (filter === 'pending') filtered = tFiltered.filter(s =>
                                 s.approvalStatus === 'pending_approval');
    else if (filter === 'rejected') filtered = tFiltered.filter(s =>
                                 s.approvalStatus === 'rejected');
    else if (filter === 'draft') filtered = tFiltered.filter(s =>
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <>
              ...
              <div className="flex gap-3 mb-8 overflow-x-auto pb-2 no-scrollbar">
                ...
                {(['all', 'published', 'pending', 'rejected', 'draft'] as const).map(f =>
                    ...
                    const label = f === 'rejected' ? 'Amend Required' : f === 'all' ? 'All' : f;
                    return (
                        <button
                            key={f}
                            onClick={() =>
      ...
      {isLoading ? (
                  ...
                  <div key={i} className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm flex flex-col sm:flex-row items-center gap-6">
                      ...
                      <div className="flex-1 min-w-0 text-center sm:text-left space-y-3">
                          ...
                          <div className="flex flex-wrap justify-center sm:justify-start items-center gap-3 mt-2">
                              ...
                              <div className="h-4 w-10 bg-gray-100 rounded-lg">
                                </div>
                          </div>
          ...
          <div className="text-center py-20 bg-gray-50/50 rounded-[2.5rem] border-2 border-dashed border-gray-100">
              ...
              <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm grayscale opacity-30">
                                🏝️</div>
              <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">
                                No experiences found.</p>
          </div>
          ...
          <div className="text-center py-20 bg-gray-50/50 rounded-[2.5rem] border-2 border-dashed border-gray-100">
              ...
              <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm grayscale opacity-30">
                                🏝️</div>
              <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">
                                No cycles found in this registry view.</p>
          </div>
                                
      )}

      <CreateTypeModal isOpen={isCreateTypeOpen} onClose={() =>
