...
export default function AdminNotifications() {
    // Navigation tab state matching AdminPartners style
    const [activeTab, setActiveTab] = useState<'writeups' | 'system' | 'tracker'>('writeups');
    ...
    // Users and Multi-Select Target states
    ...
    const targetOptions: MultiSelectOption[] = useMemo(() => {
        return users.map(u => ({
            ...
            label: u.name,
            sublabel: u.email,
            badge: u.fcmToken && !u.notificationsDisabled ? 'Subscribed' : u.notificationsDisabled ? 'Disabled' : 'No Token',
            isSubscribed: !!(u.fcmToken && !u.notificationsDisabled)
        }));
    }, [users]);

    const subscribedUserIds = useMemo(() => {
        return users.filter(u => u.fcmToken && !u.notificationsDisabled).map(u => u.id);
    }, [users]);
    ...
    // 2. Fetch users for the target dropdown
    useEffect(() => {
    ...
    // Send writeup to selected targets in real-time
    const handleSendToSelectedTargets = async (writeup: Writeup) => {
        if (selectedUserIds.length === 0) {
            alert('Please select at least one target user first!');
            return;
        }

        const actionKey = `${writeup.id || 'custom'}-selected`;
        ...
        try {
            ...
            setSendingStates(prev => ({
                ...
                [actionKey]: {
                    status: 'success',
                    message: `Dispatched to ${res.sentCount || 0} user(s)! ${res.failedCount ? `(${res.failedCount} failed)` : ''}`
                }
            }));
    ...
    // Instant real-time broadcast to all subscribed users
    const handleSendToAllSubscribers = async (writeup: Writeup) => {
        ...
        if (!window.confirm(`Are you sure you want to instantly broadcast this push notification to all ${subscribedUserIds.length} subscribed users in real-time?`)) {
            return;
        }
        ...
        try {
            ...
            setSendingStates(prev => ({
                ...
                [actionKey]: { 
                    status: 'success', 
                    message: `Broadcast complete! Sent: ${res.sentCount || 0}, Failed: ${res.failedCount || 0}` 
                }
            }));
            ...
            setSendingStates(prev => ({
                ...prev,
                [actionKey]: { status: 'error', message: error.message || 'Broadcast failed' }
            }));
        }
    };
    ...
    // Tracking stats metrics
    const deliveredLogsCount = useMemo(() => logs.filter(l => l.status === 'delivered').length, [logs]);
        ...
        <div className="space-y-6 animate-fade-in text-left pb-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm text-left">
                    ...
                    <p className="text-xl font-black text-primary mt-1">{writeups.length}</p>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm text-left">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Subscribed Users</p>
                    <p className="text-xl font-black text-blue-600 mt-1">{subscribedUserIds.length}</p>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm text-left">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Sent Dispatches</p>
                    <p className="text-xl font-black text-indigo-600 mt-1">{sentLogsCount}</p>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm text-left">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Delivered Dispatches</p>
                    <p className="text-xl font-black text-green-600 mt-1 flex items-center gap-1.5">
                        <span>{deliveredLogsCount}</span>
                        {sentLogsCount > 0 && (
                            <span className="text-[10px] font-bold text-green-700 bg-green-50 px-2 py-0.5 rounded-full border border-green-100">
                                {Math.round((deliveredLogsCount / sentLogsCount) * 100)}%
                            </span>
                        )}
                    </p>
                </div>
                    ...
                    <div className="lg:col-span-7 text-left space-y-4">
                        ...
                        <div className="bg-gray-50/70 p-4 rounded-2xl border border-gray-100 space-y-2">
                            <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest">
                                🎯 Select Target Users (Multi-Select)
                            </label>
                            <CustomMultiSelect
                                options={targetOptions}
                                selectedValues={selectedUserIds}
                                onChange={setSelectedUserIds}
                                placeholder="Choose target users..."
                                onSelectAllSubscribed={() => setSelectedUserIds(subscribedUserIds)}
                            />
                            <div className="flex justify-between items-center text-[9px] font-bold text-gray-400 pt-1">
                                <span>{selectedUserIds.length} user(s) currently targeted</span>
                                {subscribedUserIds.length > 0 && (
                                    <button
                                        type="button"
                                        onClick={() => setSelectedUserIds(subscribedUserIds)}
                                        className="text-primary hover:underline cursor-pointer font-black uppercase tracking-wider"
                                    >
                                        Select All Subscribed ({subscribedUserIds.length})
                                    </button>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 max-h-[480px] overflow-y-auto pr-2 no-scrollbar">
                            {writeups.map((w) => {
                                const selectedStateKey = `${w.id || 'custom'}-selected`;
                                const allStateKey = `${w.id || 'custom'}-all`;
                                
                                const selectedState = sendingStates[selectedStateKey];
                                const allState = sendingStates[allStateKey];
                                    ...
                                    >
                                        ...
                                        <div className="flex sm:flex-col items-stretch sm:items-end gap-2 w-full sm:w-auto flex-shrink-0">
                                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                                <button
                                                    type="button"
                                                    onClick={() => handleSendToSelectedTargets(w)}
                                                    disabled={selectedUserIds.length === 0 || selectedState?.status === 'sending'}
                                                    ...
                                                    title="Send real-time push notification to selected targets"
                                                >
                                                    <Send className="w-3 h-3" />
                                                    <span>
                                                        {selectedState?.status === 'sending' 
                                                            ? 'Sending...' 
                                                            : `Send Target (${selectedUserIds.length})`}
                                                    </span>
                                                </button>
                                                ...
                                                <button
                                                    ...
                                                    disabled={allState?.status === 'sending' || subscribedUserIds.length === 0}
                                                    className="flex-1 sm:flex-none px-3.5 py-2 bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-sm"
                                                    title="Broadcast real-time push to all active subscribed users"
                                                >
                                                    <Users className="w-3 h-3" />
                                                    <span>
                                                        {allState?.status === 'sending' ? 'Broadcasting...' : 'Broadcast All'}
                                                    </span>
                                                </button>
                ...
                <div className="text-left animate-fade-in space-y-6 pt-2">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-gray-100 pb-4">
                        <div>
                            ...
                            <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mt-1">
                                Track both Sent and Delivered status in real-time.
                            </p>
                        </div>
                    ...
                    <div className="flex gap-2 mb-6 overflow-x-auto no-scrollbar">
                        {[
                            ...
                            { id: 'sent', label: `Sent (${logs.filter(l => l.status === 'sent').length})` },
                            ...
                            { id: 'unsubscribed', label: `Unsubscribed (${logs.filter(l => l.status === 'unsubscribed').length})` },
                            { id: 'disabled', label: `Disabled (${logs.filter(l => l.status === 'disabled').length})` }
                        ].map(tab => (
                            <button
                                key={tab.id}
                                type="button"
