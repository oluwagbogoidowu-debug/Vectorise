...
const SprintView: React.FC<SprintViewProps> = ({ isPreview = false, previewSprintId }) => {
  ...
  // Force Guided Mode to start from Move 1 when active and reset slide index
  useEffect(() => {
  ...
  const getLinkedTagsForStep = (stepIndex: number): string[] => {
    ...
    if (stepIndex > 0 && dayContent.taskLinkedToNext?.[stepIndex - 1] === true) {
      ...
      const srcType = getStepInputType(dayContent, prevIdx, taskInputs, sprint?.dailyContent, enrollment?.progress);
      if (isStepOrSubStepPoll(rawSrcType) || srcType === "poll" || srcType === "tags" || srcType.includes("tags") || srcType === "dual") {
        const val = (taskInputs && taskInputs[prevIdx]) || enrollment?.progress?.find((p: any) => Number(p.day) === Number(dayContent.day || 1))?.answers?.[prevIdx];
        if (val) {
          allTags.push(...parseAnswerValues(val, srcType));
        } else {
          const configuredOpts = getAllStepPollOptions(dayContent, prevIdx, taskInputs, sprint?.dailyContent, enrollment?.progress);
          if (configuredOpts.length > 0) {
            allTags.push(...configuredOpts);
          }
        }
  ...
  const handleTouchEnd = () => {
    ...
    const text = dayContent?.lessonText || "";
        ...
        <div className="page-content w-full min-h-screen bg-white flex flex-col font-sans text-dark animate-fade-in pb-24">
          <header className="px-6 sm:px-12 md:px-16 pt-8 pb-4 max-w-5xl mx-auto w-full sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100/80">
            <div className="flex items-center justify-between">
              ...
              <div className="flex-1 min-w-0 flex justify-center px-4">
                <span className="px-3.5 py-1.5 bg-[#0E7850]/5 text-[#0E7850] rounded-2xl text-[10px] font-black uppercase tracking-widest border border-[#0E7850]/10 text-center truncate max-w-xs sm:max-w-md">
                  {sprint.title}
                </span>
              </div>

              <div className="flex items-center gap-2">
                {/* Move outside the card, Move, Card having 5 */}
                <div className="flex items-center gap-1.5 select-none shrink-0 font-black">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[#0E7850]">Move</span>
                  <div className="w-10 h-10 rounded-2xl bg-[#0E7850] text-white flex items-center justify-center shadow-md font-black text-sm">
                    {viewingDay}
                  </div>
                </div>
          ...
          <div className="px-6 sm:px-12 md:px-16 max-w-5xl mx-auto w-full flex-1 flex flex-col pt-8">
            ...
            {enrollment.status === "queued" ? (
              ...
              const slides = (dayContent?.lessonText || "")
                .split(/\n\s*\n/)
                .map((s) => s.trim())
                .filter(Boolean);
              return (
                <div 
                  className="flex-1 flex flex-col justify-between select-none relative min-h-[50vh] pb-24"
                  onTouchStart={handleTouchStart}
                  onTouchMove={handleTouchMove}
                  onTouchEnd={handleTouchEnd}
                  onClick={handleSlideClick}
                >
                  {/* Today's Insight Slide Area */}
                  <div className="flex-1 flex flex-col justify-center max-w-3xl mx-auto w-full py-8 text-center animate-fade-in cursor-pointer">
                    <p className="text-[11px] font-black uppercase tracking-[0.35em] text-[#0E7850] mb-6 select-none">
                    ...
                    <AnimatePresence mode="wait">
                      <motion.div
                        ...
                        className="text-gray-950 font-normal text-xl sm:text-2xl md:text-3xl leading-[1.8] tracking-normal text-center select-none"
                      >
                        <FormattedText text={slides[activeSlideIndex] || ""} />
                      </motion.div>
                    </AnimatePresence>
                  </div>

                  {/* Swipe to continue is fix at the bottom center */}
                  <div className="fixed bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2.5 z-40 select-none pointer-events-auto">
                      ...
                      {slides.map((_, idx) => (
                        <button
                          ...
                          className={`h-1.5 rounded-full transition-all duration-300 ${
                            idx === activeSlideIndex ? "w-6 bg-[#0E7850]" : "w-1.5 bg-gray-200"
                          }`}
                          title={`Slide ${idx + 1}`}
                        />
                      ))}
                    </div>
                    <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[#0E7850] animate-pulse">
          ...
          <div className="fixed bottom-0 left-0 right-0 h-1.5 bg-gray-100/90 backdrop-blur-sm z-50 select-none">
            <div 
              ...
              style={{
                width: `${
                  guidedStage === "insight" 
                    ? (((activeSlideIndex + 1) / ((dayContent?.lessonText || "").split(/\n\s*\n/).map(s => s.trim()).filter(Boolean).length || 1)) * 30) 
                    : 30 + ((activeTaskIndex + 1) / (getTotalVisibleStepsCount() || 1)) * 70
                }%`
              }}
            ></div>
          </div>
                            ...
                            >
                              ...
                              <div className="absolute top-6 left-6 sm:left-12 right-6 sm:right-12 z-55 flex items-center justify-between pointer-events-none">
                                ...
                                <div className="pointer-events-auto flex items-center gap-2">
                                  {sprintMode === "guided" ? (
                                    <span className="hidden sm:inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-[#0E7850]/5 text-[#0E7850] rounded-xl text-[10px] font-black uppercase tracking-widest border border-[#0E7850]/10 truncate max-w-xs sm:max-w-md">
                                      {sprint.title}
                                    </span>
                                  ) : (
                                    <button
                              ...
                              {i === activeTaskIndex && (
                                <div className="mt-6 flex flex-col gap-4 w-full">
                                  <div className="flex justify-between items-center gap-4 w-full">
                                    {getPrevVisibleStepIndex(i) !== -1 && sprintMode !== "guided" ? (
                                      <button
                                        type="button"
                                        onClick={() => {
                                          saveParticipantInputImmediately(taskInputs);
                                      ...
                                      (() => {
                                        ...
                                        return (
                                          <button
                                            ...
                                            disabled={!isValid}
                                            className={`${isFullBleed ? 'px-8 sm:px-10 py-3.5 sm:py-4 rounded-2xl text-sm sm:text-base font-black' : 'px-6 py-2.5 rounded-xl text-xs font-bold'} transition-all ${isValid ? "bg-[#0E7850] text-white hover:shadow-lg hover:shadow-[#0E7850]/20 cursor-pointer active:scale-95" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
                                          >
                                            Next
                                          </button>
                                        );
                                      ...
                                      (() => {
                                        ...
                                        return (
                                          <button
                                            ...
                                            className={`${isFullBleed ? 'px-8 sm:px-10 py-3.5 sm:py-4 rounded-2xl text-sm sm:text-base font-black' : 'px-6 py-2.5 rounded-xl text-xs font-bold'} transition-all ${isValid && !isSubmitting ? "bg-[#0E7850] text-white hover:shadow-lg hover:shadow-[#0E7850]/20 cursor-pointer active:scale-95" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
                                          >
                                            {dayProgress?.completed ? "Done" : "Next"}
                                          </button>
                                        );
                                      })()
                                    ) : (
                              ...
                              {isFullBleed && getNextVisibleStepIndex(i) === -1 && sprintMode !== "guided" && (
