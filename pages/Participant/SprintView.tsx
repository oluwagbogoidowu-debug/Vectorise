...
const SprintView: React.FC = () => {
    ...
    const hasReflection = !!(currentDayData as any)?.reflection;
    ...
    const handleStartCompletion = () => {
        if (enrollment?.reflectionsDisabled) {
            handleFinishDay('');
        } else {
    ...
    const handleToggleReflections = async () => {
        ...
        try {
            ...
            await updateDoc(enrollRef, { reflectionsDisabled: !enrollment.reflectionsDisabled });
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const handleFinishDay = async (reflection: string) => {
        ...
        try {
            ...
            // Final day logic
            if (viewingDay === enrollment.progress.length && enrollment.progress.every((p, idx) => idx === viewingDay - 1 ? true : p.completed)) {
                navigate('/discover');
            }
        } catch (err) {
    ...
    const handleAddReflectionAfterCompletion = () => {
        setIsReflectionModalOpen(true);
    };
                                ...
                                <div className="space-y-4">
                                        ...
                                        <>
                                            ...
                                            <button 
                                                ...
                                                className={`w-full py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg transition-all active:scale-95 ${
                                                    ...
                                                    : 'bg-[#159E5B] text-white shadow-green-100'
                                                }`}
                                            >
                                                {isSubmitting ? 'Marking Complete...' : 'Complete Task & Advance'}
                                            </button>
                                        </>
                                    ) : (
                                        <button 
                                            disabled
                                            className="w-full bg-gray-400 text-white py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-none cursor-default"
                                        >
                                            Marked Complete
                                        </button>
                                    )}

                                    {/* Action Icons Below Main Button */}
                                    <div className="flex justify-center items-center gap-12 pt-6">
                                        {/* Add Reflection Icon (shown only if incomplete or complete without reflection) */}
                                        {(!hasReflection || !isDayCompleted) && (
                                            <button 
                                                onClick={handleAddReflectionAfterCompletion}
                                                className="flex flex-col items-center gap-2 group transition-all"
                                                title="Add Reflection"
                                            >
                                                <div className="w-12 h-12 bg-white rounded-full border border-gray-100 flex items-center justify-center shadow-md text-gray-400 group-hover:text-primary group-hover:border-primary group-hover:scale-105 transition-all">
                                                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
                                                    </svg>
                                                </div>
                                                <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest group-hover:text-primary">Add Reflection</span>
                                            </button>
                                        )}

                                        {/* Settings Icon (Toggle Reflections Off) */}
                                        <button 
                                            onClick={handleToggleReflections}
                                            className={`flex flex-col items-center gap-2 group transition-all ${enrollment.reflectionsDisabled ? 'opacity-100' : 'opacity-60 hover:opacity-100'}`}
                                            title={enrollment.reflectionsDisabled ? "Enable Reflections" : "Disable Reflections for this Sprint"}
                                        >
                                            <div className={`w-12 h-12 bg-white rounded-full border flex items-center justify-center shadow-md transition-all ${enrollment.reflectionsDisabled ? 'border-red-400 text-red-400' : 'border-gray-100 text-gray-400 group-hover:text-primary group-hover:border-primary group-hover:scale-105'}`}>
                                            ...
                                            <span className={`text-[9px] font-black uppercase tracking-widest ${enrollment.reflectionsDisabled ? 'text-red-400' : 'text-gray-400 group-hover:text-primary'}`}>
                                                {enrollment.reflectionsDisabled ? 'Disabled' : 'Enabled'}
                                            </span>
                                        </button>
                                    </div>
                                    ...
                                    {isDayCompleted && (
                                        <div className="space-y-6 pt-10">
                                            ...
                                            {/* SAVED REFLECTION WITH SETTING LABEL */}
                                            <div className="space-y-4 pt-6">
                                                <div className="flex flex-col items-center">
                                                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.25em] mb-4">
                                                        Reflection Mode: <span className={enrollment.reflectionsDisabled ? 'text-red-400' : 'text-primary'}>{enrollment.reflectionsDisabled ? 'OFF' : 'ON'}</span>
                                                    </p>
                                                </div>
