...
const MySprints: React.FC = () => {
    ...
    if (isLoading) {
        return (
            <div className="h-screen w-full bg-light flex flex-col overflow-hidden">
                {/* Header Skeleton */}
                <div className="px-6 py-5 flex-shrink-0 bg-white border-b border-gray-100 flex justify-between items-center">
                ...
                <div className="flex-1 overflow-y-auto px-4 py-6 pb-32 custom-scrollbar">
                    <div className="max-w-screen-lg mx-auto w-full space-y-10">
                        {/* In Progress Section Skeleton */}
                        <div>
                            <div className="h-3 w-20 bg-gray-200 rounded mb-4 animate-pulse"></div>
                            <div className="grid grid-cols-1 gap-3">
                                {[1, 2].map((i) => (
                                    <div key={i} className="bg-white rounded-2xl p-4 border border-gray-100 flex flex-col sm:flex-row gap-4 animate-pulse">
                                        ...
                                        <div className="flex-1 min-w-0 space-y-3">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <div className="h-2.5 w-8 bg-gray-100 rounded"></div>
                                            </div>
                        ...
                        {/* Upcoming Queue Section Skeleton */}
                        <div>
                            <div className="h-3 w-28 bg-gray-200 rounded mb-4 animate-pulse"></div>
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 gap-2.5">
                                {(isArchivedExpanded ? archived : archived.slice(0, 2)).map(({ id, enrollment, sprint, runNumber, totalRuns }) => {
                                    const sprintCover = getSprintCoverImage(sprint);
                                    const isChallenge = sprint.contentType === 'challenge' || sprint.challengeData || sprint.challengeType;
                                    const reviewPath = isChallenge ? `/challenge/${sprint.id}` : `/participant/sprint/${enrollment.id}`;
                                    return (
                                        <div key={id} className="bg-white rounded-xl p-3 border border-gray-100 flex items-center gap-4 group animate-fade-in">
                                            ...
                                            <div className="flex items-center gap-1">
                                                <Link 
                                                    to={reviewPath} 
                                                    state={isChallenge ? { sprint, enrollment, viewMode: 'active' } : undefined}
                                                    className="p-2 text-gray-300 hover:text-primary transition-colors hover:bg-gray-50 rounded-lg" 
                                                    title="Review"
                                                >
